...
import {
  ...
  SponsorWalletTransaction,
  SponsorWalletTransactionDocument,
} from './sponsorWallet.schema';
import { Model } from 'mongoose';

@Injectable()
export class sponsorWalletService {
  constructor(
    @InjectModel(SponsorWallet.name)
    private sponsorWalletModel: Model<SponsorWalletDocument>,
  ) {}
  ...
  async update(id: string, sponsorWalletDocument: any) {
    return this.sponsorWalletModel
      .findByIdAndUpdate(id, sponsorWalletDocument, {
...
export class sponsorWalletTransactionService {
  constructor(
    @InjectModel(SponsorWalletTransaction.name)
    private sponsorWalletTransactionModel: Model<SponsorWalletTransactionDocument>,
  ) {}
  ...
  async create(sponsorWalletTransactionDocument: any) {
    return await new this.sponsorWalletTransactionModel(
      sponsorWalletTransactionDocument,
    ).save();
  }

  async update(id: string, sponsorWalletTransactionDocument: any) {
    return await this.sponsorWalletTransactionModel
      .findByIdAndUpdate(id, sponsorWalletTransactionDocument)
      .exec();
  }
